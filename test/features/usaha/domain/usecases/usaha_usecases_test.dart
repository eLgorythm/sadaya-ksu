...
import 'package:sadaya/features/usaha/domain/usecases/chip_usecases.dart';
...
void main() {
  ...
  group('CreateMaterial', () {
    test('gagal bila nama kosong', () async {
      ...
      final result = await usecase(
        const CreateMaterialParams(name: '   ', unit: 'kg'),
      );
    ...
    test('gagal bila satuan kosong', () async {
      final usecase = CreateMaterial(repository);
      final result = await usecase(
        const CreateMaterialParams(name: 'Kentang', unit: '  '),
      );
      expect(result, isA<Err<void>>());
    });
  });
  ...
  group('RecordMaterialTransaction', () {
    ...
    test('pemakaian tidak meneruskan harga', () async {
      ...
      when(
        ...
        capturedPrice = invocation.namedArguments[#unitPrice] as double?;
        return const Ok(0.0);
      });
  ...
  group('ChipAmbilDariBank', () {
    test('gagal bila nominal <= 0', () async {
      final usecase = ChipAmbilDariBank(repository);
      final result = await usecase(
        ChipAmbilDariBankParams(
          amount: 0,
          date: fakeDate,
          description: 'Isi kas unit',
        ),
      );
      expect(result, isA<Err<void>>());
    });

    test('gagal bila keterangan kosong', () async {
      final usecase = ChipAmbilDariBank(repository);
      final result = await usecase(
        ChipAmbilDariBankParams(
          amount: 500000,
    ...
    test('gagal bila tanggal di masa depan', () async {
      final usecase = ChipAmbilDariBank(repository);
      final result = await usecase(
        ChipAmbilDariBankParams(
          amount: 500000,
          date: DateTime(2999, 1, 1),
          description: 'Isi kas unit',
        ),
      );
      expect(result, isA<Err<void>>());
    ...
    test('valid diteruskan ke repository', () async {
      when(
        () => repository.chipAmbilDariBank(
