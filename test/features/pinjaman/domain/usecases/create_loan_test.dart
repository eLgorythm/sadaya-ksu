...
void main() {
  setUpAll(() {
    registerFallbackValue(
      CreateLoanParams(memberId: '', principal: 0, tenor: 10),
    );
  });
  ...
  test('valid loan delegates to repository', () async {
    ...
    final result = await useCase(
      const CreateLoanParams(memberId: 'm-1', principal: 2000000, tenor: 10),
    );
  ...
  test('zero principal rejected', () async {
    final result = await useCase(
      const CreateLoanParams(memberId: 'm-1', principal: 0, tenor: 10),
    );
  ...
  test('tenor above 50 rejected', () async {
    final result = await useCase(
      const CreateLoanParams(memberId: 'm-1', principal: 1000000, tenor: 60),
    );
