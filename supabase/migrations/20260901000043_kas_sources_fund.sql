-- =============================================================
-- Sadaya — Migrasi 43: Tab Kas ikut menampilkan pemasukan dana manual
--
-- post_fund_entry (migrasi 42) mencatat kas masuk manual dengan
-- source_book = 'fund' (baris Kas 1111 debit berpasangan dengan
-- kredit akun pos). get_cash_sources sebelumnya hanya mengambil
-- source_book = 'installment', sehingga pemasukan manual tidak
-- terlihat di kartu sumber tab Kas, padahal saldo berjalan & Buku
-- Besar sudah tercatat.
--
-- Perbaikan: masukkan source_book = 'fund' ke filter baris Kas.
-- Kas keluar manual otomatis tidak muncul karena baris posnya
-- berdebit (credit_amount = 0).
-- =============================================================
