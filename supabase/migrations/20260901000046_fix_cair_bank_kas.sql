-- =============================================================
-- Sadaya — Migrasi 46: Perbaikan "Cair dari Bank" tidak tampil
-- di Buku Kas (tab Kas).
--
-- Masalah:
--   bank_cair_ke_kas (migrasi 41) memanggil
--   create_cash_book_transaction(p_book='bank', ...) yang
--   membuat jurnal dengan source_book = 'bank'.
--   Namun get_cash_sources (migrasi 43) mencari
--   source_book = 'cash' pada baris akun 1111 — akibatnya
--   kartu "Cair dari Bank" selalu 0.
--
-- Perbaikan:
--   Ubah filter v_bank agar mencari source_book = 'bank'
--   (bukan 'cash') dengan syarat ada pasangan akun 1112
--   pada reference_id yang sama. Ini cocok dengan cara
--   bank_cair_ke_kas mencatat jurnal.
-- =============================================================
